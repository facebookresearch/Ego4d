...
def run_feature_extraction(path: str, model: nn.Module, feature_extract_config: FeatureExtractConfig):
    ...
    feature_extract_config.inference_config.stride = int(np.round(float(fps * (16/30))))
    feature_extract_config.inference_config.frame_window = int(np.round(float(fps * (32/30))))
