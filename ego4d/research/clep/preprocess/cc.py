...
def _map_cc_batch(batch_idx, cc: CCPreprocessConfig, feature_extract_config: FeatureExtractConfig):
    idx, batch = batch_idx
...
def _map_cc_sent_batch(batch_idx: Tuple[int, List[str]], config: TrainConfig, cc: CCPreprocessConfig):
    idx, batch = batch_idx
    paths = [x for x, _ in batch]
