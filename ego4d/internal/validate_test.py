...
def test_validate_video_components():
    ...
    validate._validate_video_components(s3, bucket_name, meta_dict, component_dict, err)
    assert err[1].description == 'video_metadata has component index 2 when it should have 1'
    assert err[2].description == 'video s3://ego4d-universityaf/Participant 4/0278/2.mp4 doesn\'t exist in bucket'
    
    component_dict = {uid2: [component2, component1]}
    validate._validate_video_components(s3, bucket_name, meta_dict, component_dict, err)
    assert err[3].description == 'video_component_file points to video_metadata at non-existent university_video_id'
...
def test_get_videos():
    ...
    assert err[0].description == f"video s3://{bucket_name}/{broken_obj} can't be read by FFMPEG"
    assert len(err) == 1
