...
class FisheyeRadTanThinPrism {
 public:
  ...
  static ARK_HOST_DEVICE Eigen::Matrix<typename D::Scalar, 2, 1> project(
    ...
    if (d_param || d_point) {
      ...
      if (d_param) {
        ...
        if (numK > 0) {
          if (useSingleFocalLength) {
            temp = (params[0] * th_divr) * duvDistorted_dxryr * ab;
          } else {
        ...
        if (useTangential) {
          if (useSingleFocalLength) {
            d_param->template middleCols<2>(startP).noalias() =
                T(2) * params[0] * xr_yr * xr_yr.transpose();
            const T temp3 = params[0] * xr_yr_squaredNorm;
            (*d_param)(0, startP) += temp3;
            (*d_param)(1, startP + 1) += temp3;
          } else {
            d_param->template middleCols<2>(startP).noalias() =
                T(2) * xr_yr.cwiseProduct(params.template head<2>()) * xr_yr.transpose();
            (*d_param)(0, startP) += params[0] * xr_yr_squaredNorm;
            (*d_param)(1, startP + 1) += params[1] * xr_yr_squaredNorm;
          }
        }

        if (useThinPrism) {
          d_param->template block<1, 2>(0, startS) = params[0] * radialPowers2And4;
          d_param->template block<1, 2>(1, startS).setZero();
          d_param->template block<1, 2>(0, startS + 2).setZero();
          if (useSingleFocalLength) {
            d_param->template block<1, 2>(1, startS + 2) = params[0] * radialPowers2And4;
          } else {
            d_param->template block<1, 2>(1, startS + 2) = params[1] * radialPowers2And4;
          }
        }
      }
    }
  ...
  static ARK_HOST_DEVICE Eigen::Matrix<typename D::Scalar, 2, 1> unprojectUnitPlane(
    ...
    // we don't need to normalize as ray.z == 1;
    Eigen::Matrix<T, 3, 1> ray = unproject(uvPixel, params);
    ...
    if (d_uvPixel) {
      ...
      (*d_uvPixel) = J_point.template topLeftCorner<2, 2>().inverse();
    }
  ...
  ARK_HOST_DEVICE static void scaleParams(typename DP::Scalar s, Eigen::DenseBase<DP>& params) {
    EIGEN_STATIC_ASSERT(
        DP::ColsAtCompileTime == 1 &&
            (DP::RowsAtCompileTime == kNumParams || DP::RowsAtCompileTime == Eigen::Dynamic),
        THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
 ...
 private:
  ...
  ARK_HOST_DEVICE_INLINE static Eigen::Matrix<T, 2, 1> compute_xr_yr_from_uvDistorted(
  ...
  ARK_HOST_DEVICE_INLINE static T getThetaFromNorm_xr_yr(
  ...
  template <typename D>
  ARK_HOST_DEVICE_INLINE static void compute_duvDistorted_dxryr(
  ...
  // these are the indices within the params vector where each of the paramaters begins
