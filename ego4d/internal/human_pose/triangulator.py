        ...
        for human_name in sorted(self.pose2d.keys()):
            ...
            for keypoint_idx in range(self.num_keypoints):
                ...
                for view_idx, camera_name in enumerate(self.pose2d[human_name].keys()):
                    point_2d = self.pose2d[human_name][camera_name][keypoint_idx, :2] ## chop off the confidnece
                    ...
                    if confidence > self.keypoint_thres:
                        ...
                        choosen_cameras.append(camera_name) ## camera choosen for triangulation for this point
                ...
                if len(points) >= self.min_views:
                    ...
                    if debug == True:
                        print('kps_idx:{} kps_name:{} kps_error:{}, inliers:{}, {}'.format(keypoint_idx, COCO_KP_ORDER[keypoint_idx], \
                                                                                           reprojection_error_vector.mean(), len(inlier_views), [choosen_cameras[index] for index in inlier_views]))
