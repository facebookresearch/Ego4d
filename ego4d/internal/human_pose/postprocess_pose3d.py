...
def detect_outliers_and_interpolate(poses):
    ...
    refine_poses = fill_missing_keypoints(refine_poses) ## fill in missing keypoints using interpolation
...
def fix_smoothing_mistakes(poses):
    ...
    for i in range(num_keypoints):
        ...
        print('keypoint:{}, average_offset:{}. outlier_t:{}'.format(i, average_offset, mistake_timestamps))
...
def fix_limb_mistakes(poses, std_thres=6):
    ...
    for t in range(total_time):
        ...
        assert(validity.sum() == num_keypoints) ## all keypoints should be visible for all timesteps after filling through interpolation
    ...
    for limb_idx, limb_name in enumerate(COCO_SKELETON.keys()):
        ...
        print('{}, mean:{}. std:{}, upper:{}, lower:{}, outlier_t:{}'.format(limb_name, df.mean(), df.std(), upper_limit, lower_limit, mistake_timestamps))
