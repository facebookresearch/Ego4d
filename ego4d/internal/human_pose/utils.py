...
def check_and_convert_bbox(bbox_2d_all, image_width, image_height, bbox_thres=0.005, max_aspect_ratio_thres=5.0, min_aspect_ratio_thres=0.5):
    is_valid = (bbox_2d_all[:, 0] >= 0) * (bbox_2d_all[:, 0] <= image_width) * \
                    (bbox_2d_all[:, 1] >= 0) * (bbox_2d_all[:, 1] <= image_height)
    ...
    bbox_2d_xyxy = np.array([round(x1), round(y1), round(x2), round(y2)]) ## convert to integers

    return bbox_2d_xyxy

def draw_points_2d(image, points_2d, color=(255, 0, 0), radius=2):
    for point_2d in points_2d:
        image = cv2.circle(image, (int(point_2d[0]), int(point_2d[1])), radius, color, -1)
