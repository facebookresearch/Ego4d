...
from ffmpeg_utils import VideoInfo, get_video_info
        ...
        for i in range(len(components)):
            ...
            if bucket != bucket_name:
                error_message.append(ErrorMessage(
                    video_id,
                    "Error",
                    "video_metadata has wrong bucket name"
                ))
...
def _validate_mp4(
    video_info_dict: Dict[str, List[VideoInfo]],
    error_message: List[ErrorMessage]
) -> None:
    """
    ...
    for video_id, video_infos in video_info_dict.items():
        ...
        for i, video_info in enumerate(video_infos):
            ...
            if video_info.sample_width != None and video_info.sample_height != None:
                ...
                if video_info.sample_width < video_info.sample_height and video_info.rotate==None:
            ...
            if video_info.mp4_duration == None:
                ...
                video_length = video_info.vstart + video_info.vduration if video_info.vstart != None else video_info.vduration
                audio_length = video_info.astart + video_info.aduration if video_info.astart != None else video_info.aduration
            ...
            for i in range(len(components)):
                ...
                if i != component.component_index:
                    error_message.append(ErrorMessage(
                        video_id,
                        "Error",
                        f"video_metadata has auxilliary component index {component.component_index}"
                        f" when it should have {i}"
                    ))
...
def validate_university_files(  # noqa :C901
    ...
    video_info_param = _validate_video_components(s3, bucket_name,  video_metadata_dict, video_components_dict, error_message)
    video_info_dict = _get_videos(s3, bucket_name, video_info_param, error_message)
    _validate_mp4(video_info_dict, error_message)
    _validate_synchronized_videos(video_metadata_dict, synchronized_video_dict, error_message)
    _validate_auxilliary_videos(video_metadata_dict, auxiliary_video_component_dict, component_types, error_message)
    ...
    _validate_video_metadata(video_metadata_dict, component_types, participant_dict, scenarios, devices, physical_setting_dict, video_components_dict, error_message)
