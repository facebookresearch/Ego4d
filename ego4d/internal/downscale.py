...
DEMONSTRATOR_TRAIN_DIR = "/checkpoint/miguelmartin/demonstrator_cvpr_dataset/demonstrator_cvpr_train.json"
DEMONSTRATOR_VAL_DIR = "/checkpoint/miguelmartin/demonstrator_cvpr_dataset/demonstrator_cvpr_val.json"
DEMONSTRATOR_TEST_DIR = "/checkpoint/miguelmartin/demonstrator_cvpr_dataset/demonstrator_cvpr_test.json"
...
def call_ffmpeg(paths):
    ...
    cmd = [
        ...
        '-vf', 'scale=w=if(lt(iw\,ih)\,448\,-2):h=if(lt(iw\,ih)\,-2\,448)',
        # '-c:a', 'copy',
        # '-c:v', 'h264_nvenc',
        # '-b:v', '5M',
        tgt_path,
...
def process_all(paths):
    ...
    with ThreadPoolExecutor(5) as pool:
        for _ in tqdm(pool.map(map_fn, paths), total=len(paths), desc="Processing takes"):
            continue
...
def main():
    ...
    job_inputs = batch_it(map_values, batch_size=math.ceil(len(map_values) / num_machines))
