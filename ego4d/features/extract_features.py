    ...
    for i, x in enumerate(data):
        ...
        with torch.no_grad():
            ...
            if not config.io.debug_mode:
                ...
                vid_inp = x["video"]
                fv = None
                for batch_idx, (video_name, clip_index) in enumerate(zip(x["video_name"], x["clip_index"])):
                    to_dir = f"{config.io.debug_path}/{video_name}"
                    to_path = f"{to_dir}/{clip_index}.jpg"
                    os.makedirs(to_dir, exist_ok=True)
                    grid = torchvision.utils.make_grid(vid_inp[batch_idx].permute(1, 0, 2, 3), nrows=vid_inp.shape[2] // config.inference_config.stride)
